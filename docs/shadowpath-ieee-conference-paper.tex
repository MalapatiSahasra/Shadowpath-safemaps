...
\title{Shadow Path: A Safety-Aware Route Recommendation System for Night-Time Pedestrian Navigation Using OpenStreetMap, Streetlight Data, and Urban Activity Signals\\}
...
The recommended route $r^\ast$ is rendered as alternating covered/uncovered polyline segments -- grouping consecutive points of equal $\text{covered}(\cdot)$ value into contiguous runs -- while the remaining alternative is rendered as a lower-emphasis comparison route.
